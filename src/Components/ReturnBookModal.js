...
const ReturnBookModal = ({ student, isOpen, onClose }) => {
  ...
  const fetchIssuedBooks = async () => {
    try {
      const response = await fetch(`http://15.207.102.222:5000/v2/api/student/${student.id}/issued-books`, {
        method: 'GET',
  ...
  const handleReturnBooks = async () => {
    try {
      ...
      const response = await fetch('http://15.207.102.222:5000/v2/api/return/books', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
